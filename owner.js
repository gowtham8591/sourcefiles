...
angular.module('vkApp')
    ...
    $scope.gridOptions = {
        ...
        headerRowHeight: 35,
        aggregateTemplate: '<div ng-click="row.toggleExpand()" ng-style="rowStyle(row)" class="ngAggregate"> <span class="ngAggregateText">{{row.label}} ({{row.totalChildren()}} {{AggItemsLabel}} {{AggItemsLabel}})</span> <div style="float:right;margin-right:500px;"><button ng-click="addNewPlant(row)"> Add Plant </button></div> </div>',
    ...
    _.each(allEles, function(ele, index){
        ele.setAttribute('id', 'owner'+index);
    });
